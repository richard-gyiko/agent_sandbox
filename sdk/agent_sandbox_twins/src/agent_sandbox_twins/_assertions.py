"""Gmail and Drive assertion handlers for agent-sandbox."""
...
def assert_drive_no_file_under_path_prefix(assertion: dict[str, Any], context: Any) -> None:
    ...
    for file_item in drive_files:
        ...
        full_path = "/".join(
            part for part in [parent_path, str(file_item.get("name", ""))] if part
        )
...
def assert_gmail_message_has_label(assertion: dict[str, Any], context: Any) -> None:
    ...
    assert label in msg.get("labels", []), (
        f"Expected label '{label}' on message '{message_id}'"
    )
...
def assert_drive_filename_sequence_valid(assertion: dict[str, Any], context: Any) -> None:
    ...
    for day_key, values in by_day.items():
        ...
        if require_gapless:
            max_seq = max(values)
            expected_seq = list(range(1, max_seq + 1))
            assert sorted(values) == expected_seq, (
                f"Non-gapless sequence for day {day_key}: "
                f"expected {expected_seq}, got {sorted(values)}"
            )


_PKG = "agent_sandbox_twins"


def register_twin_assertions(
    *,
    register_assertion,
    register_assertion_param_schema,
) -> None:
    """Register all Gmail/Drive assertion handlers."""
    ...
    register_assertion(
        "drive.no_file_under_path_prefix", assert_drive_no_file_under_path_prefix
    )
    register_assertion_param_schema(
    ...
    register_assertion(
        "drive.filename_sequence_valid", assert_drive_filename_sequence_valid
    )
    register_assertion_param_schema(
